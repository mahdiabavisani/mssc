...
while ( (err1 > thr ) && i < maxIter )
    % Update C:
    C = updateC(Y,E,Ac,Z,inv_Sum_YY_acI,tau,alpha_C,numModalities,numSamples,alp,mu1,mu2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% $$$ Y-E - robust
%%%% Updating C:
function C = updateC(Y,E,Ac,Z,inv_Sum_YY_acI,tau,alpha_C,numModalities,numSamples,alp,mu1,mu2)

Sum_YY_denoised=zeros(numSamples,numSamples);
for i = 1:numModalities
    Y_denoised{i}=(Y{i}-E{i});
    YY_denoised=Y{i}'*Y_denoised{i}/alp(i);
    Sum_YY_denoised =Sum_YY_denoised + YY_denoised;
end

C = inv_Sum_YY_acI*(mu1*Sum_YY_denoised + mu2*(Z-Ac/mu2));
C=C-diag(diag(C));
